(function ($) {
  ...
  $.fn.scrollEnd = function (fn) {
    ...
    return this.on("scroll", function () {
      var el = isDoc ? this.documentElement : this;
      var scrollHeight = el.scrollHeight;
      var scrollTop = el.scrollTop;
      var clientHeight = el.clientHeight;
      var isBottom = Math.abs(scrollTop + clientHeight - scrollHeight) <= 1;

      // 加锁标记，只有从非底部滑到底部，才触发一次
      if (!this._scrollBottomLocked && isBottom) {
        this._scrollBottomLocked = true;
        fn();
      }
      // 向上滚动离开底部，解锁，下次到底还能触发
      if (!isBottom) {
        this._scrollBottomLocked = false;
      }
    });
  };
})(Atu);